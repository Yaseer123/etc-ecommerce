...
export default function ModalSizeGuide({ data, isOpen, onClose }: Props) {
  ...
  const handleHeightChange = (values: number | number[]) => {
    if (Array.isArray(values) && values.length >= 2) {
      setHeightRange({ min: values[0] ?? 100, max: values[1] ?? 200 });
    }
  ...
  const handleWeightChange = (values: number | number[]) => {
    if (Array.isArray(values)) {
      setWeightRange({ min: values[0] ?? 30, max: values[1] ?? 90 });
    }
    calculateSize(heightRange.max, weightRange.max);
  };
